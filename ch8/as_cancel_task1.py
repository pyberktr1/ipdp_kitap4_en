# this example shows basic task cancellation
...
async def main():
    ...
    if not task3.done():# is task3 done?
        print("timer will be cancelled...")
        task3.cancel()
    
    # we need to wait for the cancellation operation 
    # to complete
    try:
        await task3
    except asyncio.CancelledError:# we have a cancel exception
        print("timer task succesfully cancelled!")
