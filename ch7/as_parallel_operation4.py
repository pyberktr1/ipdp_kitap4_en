...
async def main():
    ...
    # we join all tasks under a task group (tg)
    async with asyncio.TaskGroup() as tg:
        task1 = tg.create_task(as_cube(2))
        task2 = tg.create_task(as_square(3))
    
    # all tasks are completed
    print(f"square : {task2.result()}")
    print(f"cube   : {task1.result()}")
